...
class ADRFLController(Controller):
    def __init__(self, Tp, q0, Kp, Kd, p):
        self.model = ManipulatorModel(Tp, m3=0.1, r3=0.5)
        self.Kp = Kp
        self.Kd = Kd
        self.last_u = np.array([[0], [0]])

        self.L = np.array(
            [[3 * p[0], 0], [0, 3 * p[1]], [3 * p[0] ** 2, 0], [0, 3 * p[1] ** 2],
             [p[0] ** 3, 0], [0, p[1] ** 3],])
    ...
    def calculate_control(self, x, q_d, q_d_dot, q_d_ddot):
        ...
        v = (self.Kp @ (q_d.reshape(2, 1) - q) + self.Kd @ (q_d_dot.reshape(2, 1) - q_dot_est) + q_d_ddot.reshape(2, 1))
